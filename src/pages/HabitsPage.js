...
export default function HabitsPage() {
    ...
    useEffect(() => {
        ...
        promise.then((a) => setListaHabitos(a.data))
        promise.catch((a) => console.log("erro", a.data.response))
    }, [])
    function displayCreateHabit() {
...
function DisplayWeekdays(props) {
    ...
    function selectDay() {
        const array = props.selectedDays
        if (isSelected === false) {
            setIsSelected(true)
            array.push(props.day)
            props.setSelectedDays(array)
        }
        else {
            setIsSelected(false)
            props.setSelectedDays(array.filter(day => day !== props.day))
        }
    }
    return (
...
function DisplayWeekdaysStatic(props) {
    ...
    return (
        <WeekdayDivStatic data-test="habit-day" days={props.days} day={props.day} >{decideDay()}</WeekdayDivStatic>
    )
}
